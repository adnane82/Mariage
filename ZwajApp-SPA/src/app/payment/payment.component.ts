...
export class PaymentComponent implements OnInit,OnDestroy,AfterViewInit {
  ...
  onChange({ error }) {
    if (error) {
      this.loader=false;
      if(error.message==='Votre numéro de carte est incomplet.')
      this.error="Votre numéro de carte est incomplet."
      if(error.message==="La date d'expiration de votre carte est incomplète.")
      this.error="La date d'expiration de votre carte est incomplète."
      if(error.message==="Le code de sécurité de votre carte est incomplet.")
      this.error="Le code de sécurité de votre carte est incomplet."
  ...
  async onSubmit() {
    ...
    if (error) {
      console.log('Probleme de paiment', error);
    } else {
